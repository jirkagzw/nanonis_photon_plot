...
path = "C:/Users/jirka/OneDrive - uochb.cas.cz/fzu/data/2023/2023-11-25/" # PTCDA Raman-SQDM 633nm laser
...
def bias_profile_pcol(i1, i2, x1, x2, gamma=1.0, contrast=False, unit="nm", save=True,bg=305,ker_size=5,sig_th=5,center_wl=632.8,plot_two_level=False, two_level_params=None):
    ...
    if unit == "eV" and plot_two_level and two_level_params is not None:
        ...
        plt.plot(Evals[:,0], V_array, lw=2, color='gray', ls='--',label='Lower eigenvalue')
        plt.plot(Evals[:,1], V_array, lw=2, color='blue', ls='--',label='Upper eigenvalue')
...
bias_profile_pcol(
    ...
    plot_two_level=True,
    two_level_params={
        "e": [1.336, 1.336+0.0027],          # onsite energies in eV
        "u": [0.0003, -0.0007],      # Stark slopes (eV/V)
        "J": [[0, 0.0008],[0.0008, 0]]  # 2x2 coupling matrix
    }
)

bias_profile_pcol(
    'LS-PL00042','LS-PL00064',
    x1=900,        # photon energy lower bound in eV
    x2=960,        # photon energy upper bound in eV
    unit="eV",
    contrast=True,
    ...
    plot_two_level=True,
    two_level_params={
        "e": [1.3293, 1.337],               # On-site energies in eV
        "u": [-0.00102, -0.00102],           # Stark slopes (eV/V)
